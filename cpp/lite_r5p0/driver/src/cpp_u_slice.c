...
slice_drv_param_t *slice_parm)
{
	...
	if (cfg_parm->scale_mode == SCALE_MODE_2OUT)
		slice_parm->bypass_path_param.enable = 1;
	...
	if (cfg_parm->scale_deci.hor > 0) {
		...
		slice_parm->deci_param.deci_x =
			cfg_parm->scale_deci.hor - 1;
	} else {
		slice_parm->deci_param.deci_x_en = 0;
		slice_parm->deci_param.deci_x = 0;
	}
	if (cfg_parm->scale_deci.ver > 0) {
		slice_parm->bypass_path_param.enable = 0;
		slice_parm->deci_param.deci_y_en = 1;
		slice_parm->deci_param.deci_y =
			cfg_parm->scale_deci.ver - 1;
	 } else {
		slice_parm->deci_param.deci_y_en = 0;
		slice_parm->deci_param.deci_y = 0;
	}
