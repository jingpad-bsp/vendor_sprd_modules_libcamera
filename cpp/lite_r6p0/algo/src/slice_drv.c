...
void slice_drv_param_calc(struct sprd_cpp_scale_slice_parm *param_ptr)
{
    ...
    struct sprd_cpp_hw_slice_parm *fw_slice_param_ptr;
    ...
    // init deci
	if(param_ptr->deci_param.hor){
		scaler_path_param.deci_x_eb = 1;
		scaler_path_param.deci_x    = param_ptr->deci_param.hor - 1;
	}
	if(param_ptr->deci_param.ver){
		scaler_path_param.deci_y_eb = 1;
		scaler_path_param.deci_y    = param_ptr->deci_param.ver - 1;
	}
	if(param_ptr->deci_param.hor){
		bypass_path_param.deci_x_eb = 1;
		bypass_path_param.deci_x    = param_ptr->deci_param.hor - 1;
	}
	if(param_ptr->deci_param.ver){
		bypass_path_param.deci_y_eb = 1;
		bypass_path_param.deci_y    = param_ptr->deci_param.ver - 1;
	}
    ...
    for (i=0;i<slice_rows;i++)
    {
        for (j=0;j<slice_cols;j++)
        {
            ...
            do
            {
				fw_slice_param_ptr->deci_param.hor= param_ptr->deci_param.hor;
				fw_slice_param_ptr->deci_param.ver = param_ptr->deci_param.ver;

                fw_slice_param_ptr->sc_in_trim_src.w   = slice_w;
                fw_slice_param_ptr->sc_in_trim_src.h  = slice_h;
                fw_slice_param_ptr->bypass_trim_src.w  = slice_w;
                fw_slice_param_ptr->bypass_trim_src.h = slice_h;
            }while(0);
            ...
            if(deci_ptr->deci_x_en == 1 || deci_ptr->deci_y_en == 1)
            {
                ...
                do
                {
                    fw_slice_param_ptr->sc_in_trim_src.w  = deci_w;
                    fw_slice_param_ptr->sc_in_trim_src.h = deci_h;
                }while(0);
            }
            ...
            do
            {
                if(1)// scaler path
                {
                    ...
                    {
                        fw_slice_param_ptr->sc_in_trim.x = scaler_path_param.slice_param.trim0_info.trim_start_x;
                        fw_slice_param_ptr->sc_in_trim.y = scaler_path_param.slice_param.trim0_info.trim_start_y;
                        fw_slice_param_ptr->sc_in_trim.w    = scaler_path_param.slice_param.trim0_info.trim_size_x;
                        fw_slice_param_ptr->sc_in_trim.h   = scaler_path_param.slice_param.trim0_info.trim_size_y;
                    }
                    else
                    {
                        fw_slice_param_ptr->sc_in_trim.x = 0;
                        fw_slice_param_ptr->sc_in_trim.y = 0;
                        fw_slice_param_ptr->sc_in_trim.w    = fw_slice_param_ptr->sc_in_trim_src.w;
                        fw_slice_param_ptr->sc_in_trim.h   = fw_slice_param_ptr->sc_in_trim_src.h;
                    }
                    fw_slice_param_ptr->sc_slice_in_width   = fw_slice_param_ptr->sc_in_trim.w;
                    fw_slice_param_ptr->sc_slice_in_height  = fw_slice_param_ptr->sc_in_trim.h;
                ...
                if(1)//bypass path
                {
                    fw_slice_param_ptr->path0_bypass_path_en = param_ptr->bypass_path_param.bypass_eb;
                    if(bypass_path_param.slice_param.trim0_info.trim_en)
                    {
                        fw_slice_param_ptr->bypass_trim.x = bypass_path_param.slice_param.trim0_info.trim_start_x;
                        fw_slice_param_ptr->bypass_trim.y = bypass_path_param.slice_param.trim0_info.trim_start_y;
                        fw_slice_param_ptr->bypass_trim.w    = bypass_path_param.slice_param.trim0_info.trim_size_x;
                        fw_slice_param_ptr->bypass_trim.h   = bypass_path_param.slice_param.trim0_info.trim_size_y;
                    }
                    else
                    {
                        fw_slice_param_ptr->bypass_trim.x = 0;
                        fw_slice_param_ptr->bypass_trim.y = 0;
                        fw_slice_param_ptr->bypass_trim.w    = fw_slice_param_ptr->sc_in_trim_src.w;
                        fw_slice_param_ptr->bypass_trim.h   = fw_slice_param_ptr->sc_in_trim_src.h;
                    }
                }
            }while(0);
            ...
            do
            {
				...
				fw_slice_param_ptr->sc_out_trim_src.w  = fw_slice_param_ptr->sc_slice_out_width;
				fw_slice_param_ptr->sc_out_trim_src.h = fw_slice_param_ptr->sc_slice_out_height;
				fw_slice_param_ptr->sc_out_trim.x   = scaler_path_param.slice_param.trim1_info.trim_start_x;
				fw_slice_param_ptr->sc_out_trim.y   = scaler_path_param.slice_param.trim1_info.trim_start_y;
				fw_slice_param_ptr->sc_out_trim.w      = scaler_path_param.slice_param.trim1_info.trim_size_x;
				fw_slice_param_ptr->sc_out_trim.h     = scaler_path_param.slice_param.trim1_info.trim_size_y;

                    fw_slice_param_ptr->path0_sc_des_width     = fw_slice_param_ptr->sc_out_trim.w;
                    fw_slice_param_ptr->path0_sc_des_height    = fw_slice_param_ptr->sc_out_trim.h;
	    ...
	    fw_slice_param_ptr->path0_sc_des_pitch     = scaler_path_param.frame_param.dst_size_x;
                ...
                if(1)//bypass path
                {
                    fw_slice_param_ptr->path0_bypass_des_width  = fw_slice_param_ptr->bypass_trim.w;
                    fw_slice_param_ptr->path0_bypass_des_height = fw_slice_param_ptr->bypass_trim.h;
