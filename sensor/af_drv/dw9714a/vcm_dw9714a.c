...
static int dw9714a_drv_create(struct af_drv_init_para *input_ptr,
    ...
    if (ret != AF_SUCCESS) {
        ...
        _dw9714a_drv_power_on(*sns_af_drv_handle, AF_TRUE);
...
static int dw9714a_drv_delete(cmr_handle sns_af_drv_handle, void *param) {
    ...
    _dw9714a_drv_power_on(sns_af_drv_handle, AF_FALSE);
...
static int _dw9714a_drv_power_on(cmr_handle sns_af_drv_handle,
    ...
    if (AF_TRUE == power_on) {
        hw_sensor_set_monitor_val(af_drv_cxt->hw_handle,
                                  dw9714a_drv_entry.motor_avdd_val);
        usleep(DW9714A_POWERON_DELAY * 1000);
    } else {
