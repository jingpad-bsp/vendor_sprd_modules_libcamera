...
static int ad5823_drv_create(struct af_drv_init_para *input_ptr,
    ...
    if (ret != AF_SUCCESS) {
        ...
        _ad5823_drv_power_on(*sns_af_drv_handle, AF_TRUE);
...
static int ad5823_drv_delete(cmr_handle sns_af_drv_handle, void *param) {
    ...
    _ad5823_drv_power_on(sns_af_drv_handle, AF_FALSE);
...
static int _ad5823_drv_power_on(cmr_handle sns_af_drv_handle,
    ...
    if (AF_TRUE == power_on) {
        hw_sensor_set_monitor_val(af_drv_cxt->hw_handle,
                                  ad5823_drv_entry.motor_avdd_val);
        usleep(AD5823_POWERON_DELAY * 1000);
    } else {
