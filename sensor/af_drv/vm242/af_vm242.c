...
#define vm242_VCM_SLAVE_ADDR (0x18 >> 1)
...
static int vm242_drv_create(struct af_drv_init_para *input_ptr,
    ...
    if (ret != AF_SUCCESS) {
        ...
        _vm242_drv_power_on(*sns_af_drv_handle, AF_TRUE);
        ret = vm242_drv_set_mode(*sns_af_drv_handle);
        if (ret != AF_SUCCESS)
            ret = AF_FAIL;
    }
...
static int vm242_drv_delete(cmr_handle sns_af_drv_handle, void *param) {
    ...
    _vm242_drv_power_on(sns_af_drv_handle, AF_FALSE);
...
static int vm242_drv_set_pos(cmr_handle sns_af_drv_handle, uint16_t pos) {
    ...
    uint16_t slave_addr = vm242_VCM_SLAVE_ADDR;
    uint16_t cmd_len = 0;
...
struct sns_af_drv_entry vm242_drv_entry = {
    .motor_avdd_val = SENSOR_AVDD_2800MV,
    .default_work_mode = 2,
    .af_ops =
        {
            .create = vm242_drv_create,
            .delete = vm242_drv_delete,
            .set_pos = vm242_drv_set_pos,
            .get_pos = NULL,
            .ioctl = vm242_drv_ioctl,
        },
};

static int _vm242_drv_power_on(cmr_handle sns_af_drv_handle,
                               uint16_t power_on) {
    CHECK_PTR(sns_af_drv_handle);
    struct sns_af_drv_cxt *af_drv_cxt =
        (struct sns_af_drv_cxt *)sns_af_drv_handle;
    ...
    if (AF_TRUE == power_on) {
        hw_sensor_set_monitor_val(af_drv_cxt->hw_handle,
                                  vm242_drv_entry.motor_avdd_val);
        usleep(VM242_POWERON_DELAY * 1000);
    } else {
...
static int vm242_drv_set_mode(cmr_handle sns_af_drv_handle) {
    ...
    slave_addr = vm242_VCM_SLAVE_ADDR;

    if (af_drv_cxt->af_work_mode) {
        mode = af_drv_cxt->af_work_mode;
    } else {
        mode = vm242_drv_entry.default_work_mode;
    }
    ...
    case 2: {
        ...
        cmd_val[1] = 0x80;
        ret_value =
        hw_Sensor_WriteI2C(af_drv_cxt->hw_handle,slave_addr,(uint8_t*)&cmd_val[0],
        cmd_len);
        if(ret_value){
                CMR_LOGE("cmd[0]:0x%x,cmd[1]:0x%x write
        failed",cmd_val[0],cmd_val[1]);
        }
        */
    } break;
    case 3:
