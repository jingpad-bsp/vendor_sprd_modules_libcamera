...
#define LOG_TAG "af_fp5510ee4"
#include "af_fp5510ee4.h"
...
static uint32_t _fp5510ee4_write_dac_code(cmr_handle sns_af_drv_handle,
    ...
    uint16_t slave_addr = FP5510EE4_VCM_SLAVE_ADDR;
    uint16_t cmd_len = 0;
...
static int fp5510ee4_drv_create(struct af_drv_init_para *input_ptr,
    ...
    if (ret != AF_SUCCESS) {
        ...
        _fp5510ee4_drv_power_on(*sns_af_drv_handle, AF_TRUE);
        ret = _fp5510ee4_drv_set_mode(*sns_af_drv_handle);
        if (ret != AF_SUCCESS)
            ret = AF_FAIL;
    }
...
static int fp5510ee4_drv_delete(cmr_handle sns_af_drv_handle, void *param) {
    cmr_int ret = AF_SUCCESS;
    CHECK_PTR(sns_af_drv_handle);
    _fp5510ee4_drv_power_on(sns_af_drv_handle, AF_FALSE);
...
static int fp5510ee4_drv_set_pos(cmr_handle sns_af_drv_handle, uint16_t pos) {
    ...
    _fp5510ee4_write_dac_code(sns_af_drv_handle, target_code);
    af_drv_cxt->current_pos = target_code;

    return ret_value;
}

static int fp5510ee4_drv_get_pos_info(struct sensor_vcm_info *info) {
    CHECK_PTR(info);
    ...
    info->slave_addr =FP5510EE4_VCM_SLAVE_ADDR;
    info->cmd_len = 2;
...
static int fp5510ee4_drv_ioctl(cmr_handle sns_af_drv_handle, enum sns_cmd cmd,
    ...
    case CMD_SNS_AF_GET_POS_INFO:
        fp5510ee4_drv_get_pos_info(param);
        break;
    default:
        break;
    }
...
struct sns_af_drv_entry fp5510ee4_drv_entry = {
    .motor_avdd_val = SENSOR_AVDD_2800MV,
    .default_work_mode = 2,
    .af_ops =
        {
            .create = fp5510ee4_drv_create,
            .delete = fp5510ee4_drv_delete,
            .set_pos = fp5510ee4_drv_set_pos,
            .get_pos = NULL,
            .ioctl = fp5510ee4_drv_ioctl,
        },
};
...
static int _fp5510ee4_drv_power_on(cmr_handle sns_af_drv_handle,
    ...
    if (AF_TRUE == power_on) {
        hw_sensor_set_monitor_val(af_drv_cxt->hw_handle,
                                  fp5510ee4_drv_entry.motor_avdd_val);
        usleep(FP5510EE4_POWERON_DELAY * 1000);
    } else {
...
static int _fp5510ee4_drv_set_mode(cmr_handle sns_af_drv_handle) {
    ...
    if (af_drv_cxt->af_work_mode) {
        ...
        mode = fp5510ee4_drv_entry.default_work_mode;
    }
