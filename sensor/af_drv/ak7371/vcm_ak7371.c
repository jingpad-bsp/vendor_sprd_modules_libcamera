...
static int vcm_ak7371_drv_create(struct af_drv_init_para *input_ptr,
    ...
    if (ret != AF_SUCCESS) {
        ...
        _ak7371_drv_power_on(*sns_af_drv_handle, AF_TRUE);
...
static int vcm_ak7371_drv_delete(cmr_handle sns_af_drv_handle, void *param) {
    ...
    _ak7371_drv_power_on(sns_af_drv_handle, AF_FALSE);
...
static int _ak7371_drv_power_on(cmr_handle sns_af_drv_handle,
    ...
    if (AF_TRUE == power_on) {
        hw_sensor_set_monitor_val(af_drv_cxt->hw_handle,
                                  vcm_ak7371_drv_entry.motor_avdd_val);
        usleep(AK7371_POWERON_DELAY * 1000);
    } else {
