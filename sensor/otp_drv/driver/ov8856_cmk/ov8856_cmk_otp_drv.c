 ...
 **/
static cmr_int _ov8856_cmk_section_checksum(cmr_u8 *buf, cmr_uint offset,
                                            cmr_uint data_count,
...
static cmr_int _ov8856_cmk_buffer_init(cmr_handle otp_drv_handle) {
...
static cmr_int _ov8856_cmk_parse_module_data(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGV("in");
...
static cmr_int _ov8856_cmk_parse_af_data(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    ...
    ret = _ov8856_cmk_section_checksum(
...
static cmr_int _ov8856_cmk_parse_awb_data(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
...
static cmr_int _ov8856_cmk_parse_lsc_data(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGV("in");
...
static cmr_int _ov8856_cmk_parse_pdaf_data(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
...
static int _ov8856_cmk_parse_ae_data(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGV("in");
...
static cmr_int _ov8856_cmk_parse_dualcam_data(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
...
static cmr_int _ov8856_cmk_awb_calibration(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    OTP_LOGV("in");
    CHECK_PTR(otp_drv_handle);
    ...
    /*
       ...
       &(ov8856_cmk_drv_entry.otp_cfg.cali_items);
...
static cmr_int _ov8856_cmk_lsc_calibration(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    OTP_LOGV("in");
    CHECK_PTR(otp_drv_handle);
...
static cmr_int _ov8856_cmk_pdaf_calibration(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    OTP_LOGV("in");
    CHECK_PTR(otp_drv_handle);
...
static cmr_int ov8856_cmk_otp_drv_create(otp_drv_init_para_t *input_para,
                                         cmr_handle *sns_af_drv_handle) {
    return sensor_otp_drv_create(input_para, sns_af_drv_handle);
}

static cmr_int ov8856_cmk_otp_drv_delete(cmr_handle otp_drv_handle) {
    return sensor_otp_drv_delete(otp_drv_handle);
}

static cmr_int ov8856_cmk_otp_drv_read(cmr_handle otp_drv_handle, void *param) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    cmr_u8 cmd_val[3];
    cmr_uint i = 0;
    char value[255];
    ...
    if (!otp_raw_data->buffer) {
        ...
        _ov8856_cmk_buffer_init(otp_drv_handle);
    }
...
static cmr_int ov8856_cmk_otp_drv_write(cmr_handle otp_drv_handle,
                                        void *p_data) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    CHECK_PTR(p_data);
...
static cmr_int ov8856_cmk_otp_drv_parse(cmr_handle otp_drv_handle,
                                        void *params) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    ...
    otp_base_info_cfg_t *base_info =
        &(ov8856_cmk_drv_entry.otp_cfg.base_info_cfg);
    otp_params_t *otp_raw_data = &(otp_cxt->otp_raw_data);
    ...
    if (sensor_otp_get_buffer_state(otp_cxt->sensor_id)) {
        ...
        _ov8856_cmk_parse_module_data(otp_drv_handle);
        _ov8856_cmk_parse_af_data(otp_drv_handle);
        _ov8856_cmk_parse_awb_data(otp_drv_handle);
        _ov8856_cmk_parse_lsc_data(otp_drv_handle);
        _ov8856_cmk_parse_pdaf_data(otp_drv_handle);
        _ov8856_cmk_parse_ae_data(otp_drv_handle);
        _ov8856_cmk_parse_dualcam_data(otp_drv_handle);
...
static cmr_int ov8856_cmk_otp_drv_calibration(cmr_handle otp_drv_handle) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGD("in");
    otp_drv_cxt_t *otp_cxt = (otp_drv_cxt_t *)otp_drv_handle;

    otp_calib_items_t *cali_items = &(ov8856_cmk_drv_entry.otp_cfg.cali_items);

    if (cali_items) {
        if (cali_items->is_pdafc && cali_items->is_pdaf_self_cal)
            _ov8856_cmk_pdaf_calibration(otp_drv_handle);
        /*calibration at sensor or isp */
        if (cali_items->is_awbc && cali_items->is_awbc_self_cal)
            _ov8856_cmk_awb_calibration(otp_drv_handle);
        if (cali_items->is_lsc && cali_items->is_awbc_self_cal)
            _ov8856_cmk_lsc_calibration(otp_drv_handle);
    }
...
static cmr_int ov8856_cmk_compatible_convert(cmr_handle otp_drv_handle,
                                             void *p_data) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGD("in");
...
/*just for expend*/
static cmr_int ov8856_cmk_otp_drv_ioctl(cmr_handle otp_drv_handle, cmr_uint cmd,
                                        void *params) {
    cmr_int ret = OTP_CAMERA_SUCCESS;
    CHECK_PTR(otp_drv_handle);
    OTP_LOGV("in");
    ...
    case CMD_SNS_OTP_DATA_COMPATIBLE_CONVERT:
        ov8856_cmk_compatible_convert(otp_drv_handle, params);
        break;
    default:
        break;
    }
