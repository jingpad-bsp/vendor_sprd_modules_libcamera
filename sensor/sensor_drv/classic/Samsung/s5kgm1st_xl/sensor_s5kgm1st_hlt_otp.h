...
#define MODULE_NAME "hlt" // module vendor name
#define MODULE_ID_s5kgm1sp_hlt                                                 \
    0x0058 // s5kgm1sp: sensor P/N;  hlt: module vendor
...
static struct otp_info_t s_s5kgm1sp_hlt_otp_info;

#define RG_RATIO_TYPICAL_s5kgm1sp_hlt 0x29c // 0x108
#define BG_RATIO_TYPICAL_s5kgm1sp_hlt 0x26e // 0x142
#define GBGR_RATIO_TYPICAL_s5kgm1sp_hlt 0x3fd
...
cmr_u8 s5kgm1sp_Sensor_OTP_read(cmr_handle handle, uint16_t otp_addr) {
    SENSOR_IC_CHECK_HANDLE(handle);
    return Sensor_readreg8bits(handle, otp_addr); // OTP data read
}
static void s5kgm1sp_hlt_enable_awb_otp(void) {
    /*TODO enable awb otp update*/
    // Sensor_writereg8bits(0x021c, 0x04 | Sensor_readreg8bits(0x021c));
}

static uint32_t s5kgm1sp_hlt_update_awb(cmr_handle handle, void *param_ptr) {
    uint32_t rtn = SENSOR_SUCCESS;
    struct otp_info_t *otp_info = (struct otp_info_t *)param_ptr;
    s5kgm1sp_hlt_enable_awb_otp();
...
static uint32_t s5kgm1sp_hlt_test_awb(void *param_ptr) {
    uint32_t flag = 1;
    struct otp_info_t *otp_info = (struct otp_info_t *)param_ptr;
    ...
    if (!strcmp(value, "on")) {
...
#if 0
        otp_info->rg_ratio_typical = RG_RATIO_TYPICAL_s5kgm1sp_hlt;
        otp_info->bg_ratio_typical = BG_RATIO_TYPICAL_s5kgm1sp_hlt;
        otp_info->gbgr_ratio_typical = GBGR_RATIO_TYPICAL_s5kgm1sp_hlt;
        ...
        otp_info->rg_ratio_typical = RG_RATIO_TYPICAL_s5kgm1sp_hlt * 1.5;
        otp_info->bg_ratio_typical = BG_RATIO_TYPICAL_s5kgm1sp_hlt * 1.5;
        otp_info->gbgr_ratio_typical = GBGR_RATIO_TYPICAL_s5kgm1sp_hlt * 1.5;
...
static uint32_t s5kgm1sp_hlt_update_lsc(cmr_handle handle, void *param_ptr) {
    uint32_t rtn = SENSOR_SUCCESS;
    SENSOR_IC_CHECK_HANDLE(handle);
    SENSOR_PRINT("s5kgm1sp_hlt_update_lsc");
...
static uint32_t s5kgm1sp_hlt_read_otp_info(cmr_handle handle, void *param_ptr,
                                           void *p_data) {
    uint32_t rtn = SENSOR_SUCCESS;
    struct otp_info_t *otp_info = (struct otp_info_t *)param_ptr;
    SENSOR_IC_CHECK_HANDLE(handle);
    ...
    reg_offset = 0x0A04;
    for (i = 0; i < 64; i++) {
        otp_buffer[i] = s5kgm1sp_Sensor_OTP_read(handle, reg_offset + i);
    ...
    /*update awb*/
    if (s5kgm1sp_hlt_test_awb(otp_info)) {
        rtn = s5kgm1sp_hlt_update_awb(handle, param_ptr);
        if (rtn != SENSOR_SUCCESS) {
            SENSOR_PRINT_ERR("OTP awb apply error!");
            return rtn;
        }
        rtn = s5kgm1sp_hlt_update_lsc(handle, param_ptr);
        if (rtn != SENSOR_SUCCESS) {
            SENSOR_PRINT_ERR("OTP awb apply error!");
            return rtn;
        }
...
static uint32_t s5kgm1sp_hlt_identify_otp(cmr_handle handle, void *param_ptr,
                                          void *p_data) {
    uint32_t rtn = SENSOR_SUCCESS;
    struct otp_info_t *otp_info = (struct otp_info_t *)param_ptr;
    SENSOR_IC_CHECK_HANDLE(handle);
    SENSOR_PRINT("Mesin enter\n");
    rtn = s5kgm1sp_hlt_read_otp_info(handle, param_ptr, p_data);
