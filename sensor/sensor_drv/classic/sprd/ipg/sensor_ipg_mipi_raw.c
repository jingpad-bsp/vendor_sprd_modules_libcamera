 ...
 */
#include "sensor_ipg_mipi_raw.h"

#undef LOG_TAG
#define LOG_TAG "ipg_mipi_raw"

#define STATIC_INFO s_ipg_static_info
#define FPS_INFO s_ipg_mode_fps_info
#define MIPI_RAW_INFO g_ipg_mipi_raw_info
#define RES_TRIM_TAB s_ipg_Resolution_Trim_Tab
#define MODULE_INFO s_ipg_module_info_tab
#define RES_TAB_RAW s_ipg_resolution_Tab_RAW

static cmr_int ipg_drv_power_on(cmr_handle handle, cmr_uint power_on) {
    SENSOR_IC_CHECK_HANDLE(handle);

    return SENSOR_SUCCESS;
}

static cmr_int ipg_drv_identify(cmr_handle handle, cmr_uint param) {
...
static cmr_int ipg_drv_write_exposure(cmr_handle handle, cmr_uint param) {
...
static cmr_int ipg_drv_read_gain(cmr_handle handle, cmr_u32 param) {
...
static cmr_int ipg_drv_set_ev(cmr_handle handle, cmr_uint param) {
 ...
 *============================================================================*/
static cmr_int ipg_drv_init_fps_info(cmr_handle handle) {
    cmr_int rtn = SENSOR_SUCCESS;
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
        ...
        for (i = 0; i < SENSOR_MODE_MAX; i++) {
            ...
            if (0 != tempfps) {
                ...
                if (fps_info->sensor_mode_fps[i].max_fps > 30) {
                    fps_info->sensor_mode_fps[i].is_high_fps = 0;
...
static cmr_int ipg_drv_get_fps_info(cmr_handle handle, cmr_u32 *param) {
    ...
    fps_info->is_high_fps = fps_data->sensor_mode_fps[sensor_mode].is_high_fps = 0;
...
static cmr_int ipg_drv_ext_func(cmr_handle handle, cmr_uint ctl_param) {
    cmr_int rtn = SENSOR_SUCCESS;
    ...
    switch (ext_ptr->cmd) {
    case 10:
        rtn = ipg_drv_set_ev(handle, ctl_param);
        break;
    default:
        break;
    }
...
static cmr_int ipg_drv_after_snapshot(cmr_handle handle, cmr_uint param) {
...
static cmr_int ipg_drv_stream_on(cmr_handle handle, cmr_uint param) {
    SENSOR_IC_CHECK_HANDLE(handle);
...
static cmr_int ipg_drv_stream_off(cmr_handle handle, cmr_uint param) {
    SENSOR_IC_CHECK_HANDLE(handle);
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int ipg_drv_access_val(cmr_handle handle, cmr_uint param) {
    ...
    case SENSOR_VAL_TYPE_GET_STATIC_INFO:
        ret = ipg_drv_get_static_info(handle, param_ptr->pval);
        break;
    case SENSOR_VAL_TYPE_GET_FPS_INFO:
        ret = ipg_drv_get_fps_info(handle, param_ptr->pval);
        break;
...
static cmr_int
ipg_drv_handle_create(struct sensor_ic_drv_init_para *init_param,
    ...
    ipg_drv_init_fps_info(sns_drv_cxt);
    /*add private here*/
    return ret;
}

static cmr_int ipg_drv_get_private_data(cmr_handle handle, cmr_uint cmd,
                                           void **param) {
    cmr_int ret = SENSOR_SUCCESS;
    SENSOR_IC_CHECK_HANDLE(handle);
    SENSOR_IC_CHECK_PTR(param);
...
static cmr_int ipg_drv_handle_delete(cmr_handle handle, void *param) {
    cmr_int ret = SENSOR_SUCCESS;
    SENSOR_IC_CHECK_HANDLE(handle);
...
static struct sensor_ic_ops s_ipg_ops_tab = {
    ...
    .delete_handle = ipg_drv_handle_delete,
    .get_data = ipg_drv_get_private_data,
    .power = ipg_drv_power_on,
    .identify = ipg_drv_identify,

    .write_exp = ipg_drv_write_exposure,
    .write_gain_value = ipg_drv_write_gain,

    /*the following interface called by oem layer*/
    .ext_ops = {
            [SENSOR_IOCTL_BEFORE_SNAPSHOT].ops = ipg_drv_before_snapshot,
            [SENSOR_IOCTL_STREAM_ON].ops = ipg_drv_stream_on,
            [SENSOR_IOCTL_STREAM_OFF].ops = ipg_drv_stream_off,
            [SENSOR_IOCTL_EXT_FUNC].ops = ipg_drv_ext_func,
            [SENSOR_IOCTL_ACCESS_VAL].ops = ipg_drv_access_val,
    }};
