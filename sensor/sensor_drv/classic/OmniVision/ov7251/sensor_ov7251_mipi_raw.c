...
#include "sensor_ov7251_mipi_raw.h"

#undef LOG_TAG
#define LOG_TAG "ov7251_mipi_raw"

#define FPS_INFO s_ov7251_mode_fps_info
#define STATIC_INFO s_ov7251_static_info
#define VIDEO_INFO s_ov7251_video_info
#define MODULE_INFO s_ov7251_module_info_tab
#define RES_TAB_RAW s_ov7251_resolution_tab_raw
#define RES_TRIM_TAB s_ov7251_resolution_trim_tab
#define MIPI_RAW_INFO g_ov7251_mipi_raw_info

static cmr_int ov7251_drv_init_fps_info(cmr_handle handle) {
...
static cmr_int
ov7251_drv_handle_create(struct sensor_ic_drv_init_para *init_param,
    ...
    ov7251_drv_init_fps_info(sns_drv_cxt);

    /*add private here*/
    return ret;
}

static cmr_int ov7251_drv_handle_delete(cmr_handle handle, void *param) {
    cmr_int ret = SENSOR_SUCCESS;
...
static cmr_int ov7251_drv_get_private_data(cmr_handle handle, cmr_uint cmd,
                                            void **param) {
    cmr_int ret = SENSOR_SUCCESS;
    SENSOR_IC_CHECK_HANDLE(handle);
...
static cmr_int ov7251_drv_power_on(cmr_handle handle, cmr_uint power_on) {
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
...
static cmr_int ov7251_drv_identify(cmr_handle handle, cmr_uint param) {
    ...
    chip_id_h = hw_sensor_read_reg(sns_drv_cxt->hw_handle, ov7251_CHIP_ID_H_ADDR);
    chip_id_l = hw_sensor_read_reg(sns_drv_cxt->hw_handle, ov7251_CHIP_ID_L_ADDR);
    SENSOR_LOGI("Identify: CHIP_ID_H = %x, CHIP_ID_L = %x", chip_id_h, chip_id_l);
    if (chip_id_h == ov7251_CHIP_ID_H_VALUE && chip_id_l == ov7251_CHIP_ID_L_VALUE) {
        SENSOR_LOGI("this is ov7251 sensor");
...
static cmr_u16 ov7251_drv_read_gain(cmr_handle handle) {
    cmr_u32 gain;
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
 ...
 *============================================================================*/
static void ov7251_drv_write_gain(cmr_handle handle, float gain) {
    float gain_a = gain;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_u16 ov7251_drv_read_frame_length(cmr_handle handle) {
    cmr_u32 frame_len;
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void ov7251_drv_write_frame_length(cmr_handle handle,
                                           cmr_u32 frame_len) {
    SENSOR_IC_CHECK_HANDLE_VOID(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_u32 ov7251_drv_read_shutter(cmr_handle handle) {
    cmr_u32 value = 0x00;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void ov7251_drv_write_shutter(cmr_handle handle, cmr_u32 shutter) {
    cmr_u16 value = 0x00;
    SENSOR_IC_CHECK_HANDLE_VOID(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
 ...
 *============================================================================*/
static cmr_int ov7251_drv_write_exposure_dummy(cmr_handle handle,
                                                cmr_u32 shutter,
                                                cmr_u32 dummy_line,
                                                cmr_u16 size_index) {
    ...
    if (dest_fr_len != cur_fr_len)
        ov7251_drv_write_frame_length(handle, dest_fr_len);
 ...
 *============================================================================*/
static cmr_int ov7251_drv_write_exposure(cmr_handle handle, cmr_uint param) {
    ...
    ret_value = ov7251_drv_write_exposure_dummy(handle, exposure_line,
 ...
 *============================================================================*/
static cmr_int ov7251_drv_write_gain_value(cmr_handle handle, cmr_uint param) {
    cmr_int ret_value = SENSOR_SUCCESS;
    float real_gain = 0;
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt *sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
    ...
    ov7251_drv_write_gain(handle, real_gain);

    return ret_value;
}

static struct sensor_reg_tag ov7251_shutter_reg[] = {
    {0x3502, 0}, {0x3501, 0}, {0x3500, 0},
};

static struct sensor_i2c_reg_tab ov7251_shutter_tab = {
    .settings = ov7251_shutter_reg, .size = ARRAY_SIZE(ov7251_shutter_reg),
};

static struct sensor_reg_tag ov7251_again_reg[] = {
    {0x350a, 0x00}, {0x350b, 0x00},
};

static struct sensor_i2c_reg_tab ov7251_again_tab = {
    .settings = ov7251_again_reg, .size = ARRAY_SIZE(ov7251_again_reg),
};

static struct sensor_reg_tag ov7251_dgain_reg[] = {
};

struct sensor_i2c_reg_tab ov7251_dgain_tab = {
    .settings = ov7251_dgain_reg, .size = ARRAY_SIZE(ov7251_dgain_reg),
};
...
static struct sensor_i2c_reg_tab ov7251_frame_length_tab = {
    .settings = ov7251_frame_length_reg,
    .size = ARRAY_SIZE(ov7251_frame_length_reg),
};

static struct sensor_aec_i2c_tag ov7251_aec_info = {
    .slave_addr = (I2C_SLAVE_ADDR >> 1),
    .addr_bits_type = SENSOR_I2C_REG_16BIT,
    .data_bits_type = SENSOR_I2C_VAL_8BIT,
    .shutter = &ov7251_shutter_tab,
    .again = &ov7251_again_tab,
    .dgain = &ov7251_dgain_tab,
    .frame_length = &ov7251_frame_length_tab,
};

static cmr_u16 ov7251_drv_calc_exposure(cmr_handle handle, cmr_u32 shutter,
...
static void ov7251_drv_calc_gain(float gain,
                                  struct sensor_aec_i2c_tag *aec_info) {
    float gain_a = gain;
    float gain_d = 0x400;
...
static cmr_int ov7251_drv_read_aec_info(cmr_handle handle, cmr_uint param) {
    cmr_int ret_value = SENSOR_SUCCESS;
    ...
    info->aec_i2c_info_out = &ov7251_aec_info;
    ...
    //        ov7251_drv_get_default_frame_length(handle, mode);
    ...
    sns_drv_cxt->sensor_ev_info.preview_shutter = ov7251_drv_calc_exposure(
        handle, exposure_line, dummy_line, mode, &ov7251_aec_info);

    gain = info->gain < SENSOR_BASE_GAIN ? SENSOR_BASE_GAIN : info->gain;
    real_gain = (float)info->gain * SENSOR_BASE_GAIN / ISP_BASE_GAIN * 1.0;
    ov7251_drv_calc_gain(real_gain, &ov7251_aec_info);
    return ret_value;
}
...
static cmr_int ov7251_drv_stream_on(cmr_handle handle, cmr_uint param) {
    ...
    /* ir1 ir2 aec */
#if 1
        ov7251_drv_write_ir1(sns_drv_cxt->hw_handle, 0x3501, 0x80);
        ov7251_drv_write_ir2(sns_drv_cxt->hw_handle, 0x3501, 0x80);
#endif
#endif
...
static cmr_int ov7251_drv_stream_off(cmr_handle handle, cmr_uint param) {
    SENSOR_LOGI("E");
...
static cmr_int ov7251_drv_get_static_info(cmr_handle handle, cmr_u32 *param) {
    cmr_int rtn = SENSOR_SUCCESS;
    ...
    if (!fps_info->is_init) {
        ov7251_drv_init_fps_info(handle);
    }
...
static cmr_int ov7251_drv_get_fps_info(cmr_handle handle, cmr_u32 *param) {
    cmr_int rtn = SENSOR_SUCCESS;
...
static cmr_int ov7251_drv_access_val(cmr_handle handle, cmr_uint param) {
    ...
    case SENSOR_VAL_TYPE_GET_STATIC_INFO:
        ret = ov7251_drv_get_static_info(handle, param_ptr->pval);
        break;
    case SENSOR_VAL_TYPE_GET_FPS_INFO:
        ret = ov7251_drv_get_fps_info(handle, param_ptr->pval);
        break;
...
static struct sensor_ic_ops s_ov7251_ops_tab = {
    .create_handle = ov7251_drv_handle_create,
    .delete_handle = ov7251_drv_handle_delete,
    .get_data = ov7251_drv_get_private_data,

    .power = ov7251_drv_power_on,
    .identify = ov7251_drv_identify,
    .ex_write_exp = ov7251_drv_write_exposure,
    .write_gain_value = ov7251_drv_write_gain_value,

    .ext_ops = {
        [SENSOR_IOCTL_BEFORE_SNAPSHOT].ops = ov7251_drv_before_snapshot,
        [SENSOR_IOCTL_STREAM_ON].ops = ov7251_drv_stream_on,
        [SENSOR_IOCTL_STREAM_OFF].ops = ov7251_drv_stream_off,
        [SENSOR_IOCTL_ACCESS_VAL].ops = ov7251_drv_access_val,
    },
};
