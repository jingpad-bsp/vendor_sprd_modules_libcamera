 ...
 *============================================================================*/
static cmr_u16 imx230_drv_update_exposure(cmr_handle handle, cmr_u32 shutter,
                                          cmr_u32 dummy_line) {
...
static cmr_int imx230_drv_ex_write_exposure(cmr_handle handle, cmr_uint param) {
    cmr_int ret = SENSOR_SUCCESS;
    cmr_u16 exposure_line = 0x00;
    cmr_u16 dummy_line = 0x00;
    cmr_u16 mode = 0x00;
    ...
    frame_interval = (cmr_u16)(
        ((exposure_line + dummy_line) * trim_tab[mode].line_time) / 1000000);
...
static void imx230_drv_calc_gain(float gain,
                                 struct sensor_aec_i2c_tag *aec_info) {
    cmr_u8 i = 0;
    cmr_u32 sensor_again = 0;
    cmr_u32 sensor_dgain = 0;
...
static cmr_int imx230_drv_read_aec_info(cmr_handle handle, cmr_uint param) {
    ...
    sns_drv_cxt->sensor_ev_info.preview_shutter = imx230_drv_calc_exposure(
...
static void imx230_drv_increase_hdr_exposure(cmr_handle handle,
    ...
    if (shutter_multiply >= ev_multiplier) {
        imx230_drv_update_exposure(
            handle, sns_drv_cxt->hdr_info.capture_shutter * ev_multiplier, 0);
        imx230_drv_write_gain(handle, sns_drv_cxt->hdr_info.capture_gain);
    } else {
        gain = sns_drv_cxt->hdr_info.capture_gain * ev_multiplier /
               shutter_multiply;
        imx230_drv_update_exposure(
            handle, sns_drv_cxt->hdr_info.capture_shutter * shutter_multiply,
            0);
...
static void imx230_drv_decrease_hdr_exposure(cmr_handle handle,
    ...
    if (gain_multiply >= ev_divisor) {
        imx230_drv_update_exposure(handle,
                                   sns_drv_cxt->hdr_info.capture_shutter, 0);
        imx230_drv_write_gain(handle,
                              sns_drv_cxt->hdr_info.capture_gain / ev_divisor);

    } else {
        shutter =
            sns_drv_cxt->hdr_info.capture_shutter * gain_multiply / ev_divisor;
...
static cmr_int imx230_drv_stream_off(cmr_handle handle, cmr_uint param) {
    ...
    if (value == 0x01) {
        ...
        if (!sns_drv_cxt->is_sensor_close) {
            frame_time =
                sns_drv_cxt->line_time_def * sns_drv_cxt->frame_length / 1000 +
                1000;
...
static cmr_int imx230_drv_get_static_info(cmr_handle handle, cmr_u32 *param) {
    cmr_int ret = SENSOR_SUCCESS;
...
static cmr_int
imx230_drv_handle_create(struct sensor_ic_drv_init_para *init_param,
...
static cmr_int imx230_drv_get_private_data(cmr_handle handle, cmr_uint cmd,
                                           void **param) {
    cmr_int ret = SENSOR_SUCCESS;
    SENSOR_IC_CHECK_HANDLE(handle);
    SENSOR_IC_CHECK_PTR(param);
