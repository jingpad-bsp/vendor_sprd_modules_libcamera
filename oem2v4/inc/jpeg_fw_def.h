...
typedef BOOLEAN (*JINF_READ_FILE_FUNC)(uint8 *buf, uint32 offset,
                                       uint32 bytes_to_read,
                                       uint32 *bytes_read_ptr);

/*function of write JPEG stream data or RGB data.*/
typedef BOOLEAN (*JINF_WRITE_FILE_FUNC)(uint8 *buf_ptr, uint32 file_offset,
                                        uint32 bytes_to_write,
                                        uint32 *written_bytes_ptr);
...
PUBLIC JPEG_RET_E JPEG_HWDecStart(uint32 num_of_rows,
                                  JPEG_DEC_OUTPUT_PARA_T *output_para_ptr);

/*decode defined number of MCU according to synchronism mode by hardware*/
PUBLIC JPEG_RET_E JPEG_HWDecStartMCUSynchro(
    uint32 num_of_rows, JPEG_DEC_OUTPUT_PARA_T *output_para_ptr);

/*read out bitstream information when current slice decoding is finished. only
 * used in switch mode*/
PUBLIC JPEG_RET_E JpegDec_FwReadoutDecInfo(uint32 *bitstrm_byte_offset_ptr,
                                           uint32 bitstrm_bfr_switch_cnt);

/*close decoder and release resource*/
PUBLIC void JpegDec_FwClose(JPEG_DEC_INPUT_PARA_T *jpeg_dec_input);

PUBLIC void JpegDec_BackupLast100ByteBs(uint8 *bitstrm_ptr,
                                        uint32 pingpang_buf_len);
...
PUBLIC JPEG_RET_E JPEG_HWEncStart(uint32 width, uint32 height,
                                  JPEG_ENC_OUTPUT_PARA_T *output_para_ptr);
