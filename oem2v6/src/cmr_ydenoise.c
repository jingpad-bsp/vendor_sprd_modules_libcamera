...
static cmr_int yde_open(cmr_handle ipm_handle, struct ipm_open_in *in,
...
static int y_proc_func(void *param) {
    struct y_denoise_handle_param *yde_param =
        (struct y_denoise_handle_param *)param;
    if (NULL == param) {
        CMR_LOGE("error param");
        return -1;
    }
    CMR_LOGI("ydenoise begin width = %d, height = %d", yde_param->width,
             yde_param->height);
    ynoise_function(yde_param->cnr_in,
                    // yde_param->cnr_out,
                    yde_param->cnr_in, // use in ptr
                    yde_param->mask, yde_param->width, yde_param->height);
    return 0;
}
...
static cmr_int yde_start(cmr_handle class_handle, cmr_uint thread_id,
    ...
    yde_start_ptr =
        (struct yde_start_param *)malloc(sizeof(struct yde_start_param));
    if (!yde_start_ptr) {
        CMR_LOGE("no mem");
        return CMR_CAMERA_NO_MEM;
    }
...
static cmr_int yde_choose_mask(cmr_uint sensor_id, int gain, int8 **mask) {
    ...
    CMR_LOGI("sensor_id = %lu gain = %d paten_cnt = %d", sensor_id, gain,
             paten_cnt);
    if (0 == sensor_id) {
        if (100 > gain)
            goto exit_gain;
...
static cmr_int yde_transfer_frame(cmr_handle class_handle,
