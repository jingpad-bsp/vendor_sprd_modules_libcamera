...
cmr_s32 _pm_yuv_ygamma_init(void *dst_gamc_param, void *src_gamc_param, void *param1, void *param_ptr2)
{
	...
	for (j = 0; j < ISP_YUV_GAMMA_NUM; j++) {
		dst_ptr->cur.gain[j] = final_points[j].y;
	}
	...
	case ISP_PM_BLK_SMART_SETTING:
		{
			...
			if ((gamc_value.value[0] != ygamma_ptr->cur_idx_weight.x0) ||
				...
				void *src[2] = { NULL };
				void *dst = NULL;
				src[0] = &ygamma_ptr->curve_tab[gamc_value.value[0]].points[0].y;
				src[1] = &ygamma_ptr->curve_tab[gamc_value.value[1]].points[0].y;
				dst = &ygamma_ptr->final_curve;

				rtn = isp_interp_data(dst, src, gamc_value.weight, SENSOR_Y_GAMMA_POINT_NUM * 2, ISP_INTERP_UINT16);
				if (ISP_SUCCESS == rtn) {
