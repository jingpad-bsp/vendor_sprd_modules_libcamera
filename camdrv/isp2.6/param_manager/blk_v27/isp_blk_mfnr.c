	...
	cmr_u32 mode_flag, cmr_u32 scene_flag)
{
	...
	struct isp_mfnr_param *dst_ptr = (struct isp_mfnr_param *)dst_mfnr_param;
...
cmr_s32 _pm_mfnr_init(void *dst_mfnr_param, void *src_mfnr_param, void *param1, void *param_ptr2)
{
	...
	struct isp_pm_nr_header_param *src_ptr = (struct isp_pm_nr_header_param *)src_mfnr_param;
	struct isp_mfnr_param *dst_ptr = (struct isp_mfnr_param *)dst_mfnr_param;
	struct isp_pm_block_header *mfnr_header_ptr = (struct isp_pm_block_header *)param1;
	UNUSED(param_ptr2);
	...
	ISP_LOGV("cur_level %d level_num %d", dst_ptr->cur_level, dst_ptr->level_num);

	rtn = _pm_mfnr_convert_param(dst_ptr, dst_ptr->cur_level, ISP_MODE_ID_COMMON, ISP_SCENEMODE_AUTO);
...
cmr_s32 _pm_mfnr_set_param(void *mfnr_param, cmr_u32 cmd, void *param_ptr0, void *param_ptr1)
{
	...
	struct isp_mfnr_param *dst_ptr = (struct isp_mfnr_param *)mfnr_param;
	...
	case ISP_PM_BLK_SMART_SETTING:
		{
			...
			if (level != dst_ptr->cur_level || nr_tool_flag[ISP_BLK_MFNR_T] || block_result->mode_flag_changed) {
				dst_ptr->cur_level = level;
				header_ptr->is_update = ISP_ONE;
				nr_tool_flag[ISP_BLK_MFNR_T] = 0;

				rtn = _pm_mfnr_convert_param(dst_ptr, dst_ptr->cur_level, header_ptr->mode_id, block_result->scene_flag);
...
{
	cmr_s32 rtn = ISP_SUCCESS;
	struct isp_mfnr_param *mfnr_ptr = (struct isp_mfnr_param *)mfnr_param;
