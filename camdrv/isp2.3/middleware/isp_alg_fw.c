...
struct isp_alg_fw_context {
	...
	struct af_info_t af_cxt;
	struct aft_info_t aft_cxt;
...
static cmr_int ispalg_af_set_cb(cmr_handle isp_alg_handle, cmr_int type, void *param0, void *param1)
{
	...
	case AF_CB_CMD_SET_END_NOTICE:
		if (ISP_ZERO == cxt->commn_cxt.isp_callback_bypass) {
			...
			ret = cxt->commn_cxt.callback(cxt->commn_cxt.caller_id, ISP_CALLBACK_EVT | ISP_AF_NOTICE_CALLBACK, isp_af, sizeof(struct isp_af_notice));
		}
		break;
	...
	case AF_CB_CMD_SET_MOTOR_STATUS:
		ret = cxt->commn_cxt.callback(cxt->commn_cxt.caller_id, ISP_CALLBACK_EVT | ISP_AF_VCM_NOTICE_CALLBACK, param0, sizeof(cmr_u32));
		break;
	default:
		break;
	}
...
static cmr_int ispalg_pdaf_set_cb(cmr_handle isp_alg_handle, cmr_int type, void *param0, void *param1)
{
	...
	case ISP_PDAF_SET_CFG_PARAM:
		for (i = 0; i < param_num; i++) {
			...
			if (cxt->mode_id[i] >= ISP_MODE_ID_CAP_0 &&
					cxt->mode_id[i] <= ISP_MODE_ID_CAP_3)
				pdaf_block_info.scene_id = ISP_MODE_CAP;
			else
				pdaf_block_info.scene_id = ISP_MODE_PRV;
...
static cmr_int ispalg_handle_sensor_sof(cmr_handle isp_alg_handle)
{
	...
	if (cxt->zsl_flag) {
		for (i = 0; i < ISP_MODE_MAX; i++)
			input_param[i].mod_id = cxt->mode_id[i];
		input.param_num = ISP_MODE_MAX;
	} else {
		/*none zsl mode, only one mode valid: prev, cap, video etc*/
		input_param[0].mod_id = cxt->mode_id[0];
		input.param_num = 1;
	}
...
static cmr_int ispalg_aem_stats_parser(cmr_handle isp_alg_handle, void *data)
{
	...
	ae_stat_ptr = &cxt->ae_cxt.aem_stats;
...
cmr_int ispalg_start_ae_process(cmr_handle isp_alg_handle)
{
	...
	ISP_LOGV("SYSTEM_TEST-ae:%u ms", (cmr_u32)(time_end - time_start));
				...
				struct awb_ctrl_calc_param *out_ptr)
{
	...
	out_ptr->stat_img.chn_img.r = cxt->ae_cxt.aem_stats.r_info;
	...
	/*zsl mode_id[0]: prev, none zsl mode_id[0]: prev, cap, video etc*/
	memset(&pm_param, 0, sizeof(pm_param));
	BLOCK_PARAM_CFG(pm_param, ISP_PM_BLK_CMC10,
			ISP_BLK_CMC10,
			cxt->mode_id[0],
			0, 0);
	io_pm_input.param_num = 1;
...
#define CMC10(n) (((n)>>13)?((n)-(1<<14)):(n))
	...
	/*zsl mode_id[0]: prev, none zsl mode_id[0]: prev, cap, video etc*/
	memset(&pm_param, 0, sizeof(pm_param));
	BLOCK_PARAM_CFG(pm_param, ISP_PM_BLK_GAMMA,
			ISP_BLK_RGB_GAMC,
			cxt->mode_id[0],
			0, 0);
	io_pm_input.param_num = 1;
...
cmr_int ispalg_awb_post_process(cmr_handle isp_alg_handle, struct awb_ctrl_calc_result *awb_output)
{
	...
	memset(&ioctl_input, 0, sizeof(ioctl_input));
	memset(ioctl_data, 0, sizeof(ioctl_data));
	memset(&awbc_cfg, 0, sizeof(awbc_cfg));
				...
				struct awb_ctrl_calc_result *awb_output)
{
	...
	time_end = ispalg_get_sys_timestamp();
	ISP_LOGV("SYSTEM_TEST-awb:%u ms", (cmr_u32)(time_end - time_start));
...
static cmr_int ispalg_prepare_atm_param(cmr_handle isp_alg_handle,
					 ...
					 struct awb_ctrl_calc_result *awb_output)
{
	...
	if (1 == cxt->smart_cxt.isp_smart_eb) {
		...
		memcpy(cxt->lsc_cxt.ae_out_stats.r_info,
		       cxt->ae_cxt.aem_stats.r_info,
		       1024 * sizeof(cmr_u32));
		memcpy(cxt->lsc_cxt.ae_out_stats.g_info,
		       cxt->ae_cxt.aem_stats.g_info,
		       1024 * sizeof(cmr_u32));
		memcpy(cxt->lsc_cxt.ae_out_stats.b_info,
		       cxt->ae_cxt.aem_stats.b_info,
		       1024 * sizeof(cmr_u32));
	...
	time_end = ispalg_get_sys_timestamp();
	ISP_LOGV("SYSTEM_TEST-smart:%u ms", (cmr_u32)(time_end - time_start));
...
cmr_int ispalg_afl_process(cmr_handle isp_alg_handle, void *data)
{
	...
	cxt->afl_cxt.ae_stats = cxt->ae_cxt.aem_stats;
...
static cmr_int ispalg_pdaf_process(cmr_handle isp_alg_handle, void *in_ptr)
{
	...
	ret = ispalg_set_stats_buffer(cxt, statis_info, DCAM_PDAF_BLOCK);
...
static cmr_int ispalg_binning_stats_parser(cmr_handle isp_alg_handle, void *data)
{
	...
	memset(cxt->binning_cxt.binning_img_data, 0,
	       cxt->binning_cxt.binnig_w * cxt->binning_cxt.binnig_h * 2);
	...
	remainder = cxt->binning_cxt.binnig_w * cxt->binning_cxt.binnig_h % 6;
...
static cmr_int ispalg_awb_init(struct isp_alg_fw_context *cxt)
{
	...
	ret = isp_pm_ioctl(cxt->handle_pm, ISP_PM_CMD_GET_INIT_AWB, NULL, &output);
...
static cmr_int ispalg_lsc_init(struct isp_alg_fw_context *cxt)
{
	...
	pm_tab_input.param_data_ptr = &pm_tab_param;
...
static cmr_int ispalg_get_awbbin_size(cmr_handle isp_alg_handle)
{
	cmr_int ret = ISP_SUCCESS;
	struct isp_alg_fw_context *cxt = (struct isp_alg_fw_context *)isp_alg_handle;
	...
	cxt->binning_cxt.binnig_w = (src_w >> binning_info->hx) & ~0x1;
	cxt->binning_cxt.binnig_h = (src_h >> binning_info->vx) & ~0x1;

	ISP_LOGV("binning w h %d %d", cxt->binning_cxt.binnig_w, cxt->binning_cxt.binnig_h);
	if (ISP_BINNING_STATIS_SIZE <=
		(cxt->binning_cxt.binnig_w * cxt->binning_cxt.binnig_h * 2)) {
		ISP_LOGE("fail to match awbbin buffer size");
		return ISP_ERROR;
	}
...
static cmr_int ispalg_awbbin_init(cmr_handle isp_alg_handle)
{
	...
	cxt->binning_cxt.binnig_w = (src_w >> binning_hx) & ~0x1;
	cxt->binning_cxt.binnig_h = (src_h >> binning_vx) & ~0x1;
	cxt->binning_cxt.binning_stats.binning_size.w = cxt->binning_cxt.binnig_w / 2;
	cxt->binning_cxt.binning_stats.binning_size.h = cxt->binning_cxt.binnig_h / 2;

	cxt->binning_cxt.binning_img_data = (cmr_u16 *)malloc(ISP_BINNING_STATIS_SIZE);
...
static cmr_u32 ispalg_init(struct isp_alg_fw_context *cxt, struct isp_alg_sw_init_in *input_ptr)
{
	...
	ISP_TRACE_IF_FAIL(ret, ("fail to do awbbin_init"));
...
static cmr_int ispalg_ae_set_work_mode(cmr_handle isp_alg_handle, cmr_u32 new_mode, cmr_u32 fly_mode, struct isp_video_start *param_ptr)
{
	...
	if (cxt->ops.awb_ops.ioctrl) {
		...
		ISP_TRACE_IF_FAIL(ret, ("fail to AWB_CTRL_CMD_GET_OTP_INFO"));
	}
	if (cxt->ops.ae_ops.ioctrl) {
...
static cmr_int ispalg_update_alsc_result(cmr_handle isp_alg_handle, cmr_handle out_ptr)
{
	...
	if ((float)cxt->lsc_cxt.full_size_width / cxt->dcam_size.w ==
	    (float)cxt->lsc_cxt.full_size_height / cxt->dcam_size.h) {
		if (cxt->zsl_flag) {
			for (i = 0; i < ISP_MODE_MAX; i++) {
				if (i == 0) {  //isp preview
	...
	/* get DCAM lsc table */
	memset(&param_data_alsc, 0, sizeof(param_data_alsc));
	BLOCK_PARAM_CFG(param_data_alsc, ISP_PM_BLK_LSC_GET_LSCTAB, DCAM_BLK_2D_LSC, cxt->mode_id[0], NULL, 0);
	input.param_num = 1;
	...
	/* get DCAM lsc info */
	memset(&param_data_alsc, 0, sizeof(param_data_alsc));
	BLOCK_PARAM_CFG(param_data_alsc, ISP_PM_BLK_LSC_INFO, DCAM_BLK_2D_LSC, cxt->mode_id[0], PNULL, 0);
	input.param_num = 1;
	input.param_data_ptr = &param_data_alsc;
	ret = isp_pm_ioctl(cxt->handle_pm, ISP_PM_CMD_GET_SINGLE_SETTING, (void *)&input, (void *)&output);
	ISP_TRACE_IF_FAIL(ret, ("fail to ISP_PM_CMD_GET_SINGLE_SETTING"));
	...
	/* get binning_src paramter */
	binning_src.img_width = cxt->commn_cxt.src.w;
	binning_src.img_height = cxt->commn_cxt.src.h;
	binning_src.grid = lsc_info_new->grid;
	binning_src.gain_width = lsc_info_new->gain_w;
	binning_src.gain_height = lsc_info_new->gain_h;
	binning_src.pm_tab0 = lsc_tab_pram_ptr->map_tab[0].param_addr;
	binning_src.tab = (cmr_u16 *)dst_gain_tmp;
	ISP_LOGV("binning src: img_width:%d, img_height:%d, grid:%d, gain_width:%d, gain_height:%d, pm_tab0:[%d,%d,%d,%d,], tab:[%d,%d,%d,%d]",
			binning_src.img_width,
			binning_src.img_height,
			binning_src.grid,
			binning_src.gain_width,
			binning_src.gain_height,
			binning_src.pm_tab0[0], binning_src.pm_tab0[1], binning_src.pm_tab0[2], binning_src.pm_tab0[3],
			binning_src.tab[0], binning_src.tab[1], binning_src.tab[2], binning_src.tab[3]);

	/* get binning tab0 */
	memset(&param_data_alsc, 0x0, sizeof(param_data_alsc));
	BLOCK_PARAM_CFG(param_data_alsc, ISP_PM_BLK_LSC_GET_LSCTAB, ISP_BLK_2D_LSC, cxt->mode_id[0], NULL, 0);
	input.param_num = 1;
	input.param_data_ptr = &param_data_alsc;
	ret = isp_pm_ioctl(cxt->handle_pm, ISP_PM_CMD_GET_SINGLE_SETTING, (void *)&input, (void *)&output);
	ISP_TRACE_IF_FAIL(ret, ("fail to ISP_PM_CMD_GET_SINGLE_SETTING"));

	isp_lsc_tab_ptr = (struct isp_2d_lsc_param *)(output.param_data->data_ptr);
	if (!isp_lsc_tab_ptr) {
		ISP_LOGE("fail to get isp binning lsc tab.");
		return ISP_ERROR;
	}
	...
	if (cxt->zsl_flag) {
		for (i = 0; i < ISP_MODE_MAX; i++) {
			BLOCK_PARAM_CFG(pm_param[i], ISP_PM_BLK_LSC_INFO, ISP_BLK_2D_LSC, cxt->mode_id[i], PNULL, 0);
			input.param_num = 1;
			input.param_data_ptr = &pm_param[i];
			ret = isp_pm_ioctl(cxt->handle_pm, ISP_PM_CMD_GET_SINGLE_SETTING, (void *)&input, (void *)&output);
			isp_lsc_info = (struct isp_lsc_info *)output.param_data->data_ptr;
			if (!isp_lsc_info) {
				ISP_LOGV("fail to get isp lsc info.");
				return ISP_ERROR;
			}

			binning.ratio = (float)cxt->dcam_size.w / cxt->commn_cxt.src.w;
			ISP_LOGV("lsc_calc Zsl-binning binning.ratio=%f", binning.ratio);
			if (i == 0 && binning.ratio != 1.0) {
				binning_dst.img_width = cxt->dcam_size.w;
				binning_dst.img_height = cxt->dcam_size.h;
				binning_dst.grid = isp_lsc_info->grid;
		...
		BLOCK_PARAM_CFG(pm_param[0], ISP_PM_BLK_LSC_INFO, ISP_BLK_2D_LSC, cxt->mode_id[0], PNULL, 0);
		input.param_num = 1;
		input.param_data_ptr = &pm_param[0];
		ret = isp_pm_ioctl(cxt->handle_pm, ISP_PM_CMD_GET_SINGLE_SETTING, (void *)&input, (void *)&output);
		isp_lsc_info = (struct isp_lsc_info *)output.param_data->data_ptr;
		if (!isp_lsc_info) {
			ISP_LOGV("fail to get isp lsc info.");
		...
		binning.ratio = (float)cxt->dcam_size.w / cxt->commn_cxt.src.w;
		ISP_LOGV("lsc_calc None zsl-binning binning.ratio=%f", binning.ratio);
		if (binning.ratio != 1.0) {
			binning_dst.img_width = cxt->dcam_size.w;
			binning_dst.img_height = cxt->dcam_size.h;
			binning_dst.grid = isp_lsc_info->grid;
