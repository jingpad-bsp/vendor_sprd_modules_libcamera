...
cmr_s32 _pm_cnr3_init(void *dst_cnr3_param, void *src_cnr3_param, void *param1, void *param2)
{
	...
	rtn = _pm_cnr3_convert_param(dst_ptr,
	...
	case ISP_PM_BLK_SMART_SETTING:
		{
			...
			if (level != dst_ptr->cur_level ||
					nr_tool_flag[ISP_BLK_CNR3_T] || block_result->mode_flag_changed) {
				dst_ptr->cur_level = level;
				header_ptr->is_update = ISP_ONE;
				nr_tool_flag[ISP_BLK_CNR3_T] = 0;

				rtn = _pm_cnr3_convert_param(dst_ptr,
					dst_ptr->cur_level, block_result->mode_flag, block_result->scene_flag);
				dst_ptr->cur.bypass |= header_ptr->bypass;
				if (ISP_SUCCESS != rtn) {
					ISP_LOGE("fail to convert pm cnr3 param !");
					return rtn;
