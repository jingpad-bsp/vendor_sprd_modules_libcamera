#ifndef __SPRD_DRE_ADAPTER_HEADER_H__
#define __SPRD_DRE_ADAPTER_HEADER_H__
...
*/
JNIEXPORT int sprd_dre_adpt_init(void **handle, int max_width, int max_height, void *param);

/*
        deinit dre adapter instance
        return value: 0 is ok, other value is failed
*/
JNIEXPORT int sprd_dre_adpt_deinit(void *handle);
...
*/
JNIEXPORT int sprd_dre_adpt_ctrl(void *handle, sprd_dre_cmd_t cmd, sprd_camalg_image_t* input, void *param);

/*
        dre adapter get running type, the output is type, such as cpu/gpu/vdsp
        return value: 0 is ok, other value is failed
*/
JNIEXPORT int sprd_dre_get_devicetype(enum camalg_run_type *type);

/*
        dre adapter set running type
        return value: 0 is ok, other value is failed
*/
JNIEXPORT int sprd_dre_set_devicetype(enum camalg_run_type type);
