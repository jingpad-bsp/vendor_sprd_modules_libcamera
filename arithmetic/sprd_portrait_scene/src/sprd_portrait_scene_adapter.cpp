...
*/
    ...
    if(prop_value){
        ...
        PScene_LOGI("Failed to set run_type by prop because of lacking memory");
    }
...
int sprd_portrait_scene_get_devicetype(camalg_run_type type){
    if (!type)
        return 1;
#ifdef CONFIG_LIB1
    *type = g_run_type;
...
static DWORD start_neg_pow2[17] = { 0, 1, 3, 7, 15, 31, 63, 127, 255, 511, 1023, 2047, 4095, 8191, 16383, 32767 };
...
void write_buf_to_BMP(BYTE *im_buffer, WORD X_bitmap, WORD Y_bitmap, char *BMPname)
{
	...
	if (fp_bitmap == NULL) PScene_LOGE("File cannot be created\n");
	...
	PScene_LOGI("Writing bitmap ...\n");
	im_loc_bytes = (DWORD)(intptr_t)im_buffer + ((DWORD)Y_bitmap - 1)*X_bitmap * 4;

	for (y = 0; y<Y_bitmap; y++)
	{
		for (x = 0; x<X_bitmap; x++)
		{
			pixel = (RGB *)(intptr_t)im_loc_bytes;
...
int load_JPEG_header(BYTE *buf, DWORD length_of_file, DWORD* X_image, DWORD* Y_image)
{
	...
	if ((Xdensity != 1) || (Ydensity != 1)) //exit_func("JPG format not supported");   
		;  //{PScene_LOGI("X density = %d\n",Xdensity); PScene_LOGI("Y density = %d\n",Ydensity);}   
...
int get_JPEG_buffer(DWORD X_image, DWORD Y_image, BYTE *address_dest_buffer)
{
	WORD y;
	DWORD dest_loc = 0;
	...
	if ((X_round == X_image) && (Y_round == Y_image))
		...
		memcpy(address_dest_buffer, im_buffer,  X_image * Y_image * 4);
	else
	{
		...
		dest_buffer = address_dest_buffer;
		for (y = 0; y<Y_image; y++) {
...
int jpg2bmp(BYTE *jpg_buf0, BYTE *bmp_buf, DWORD length_of_file, DWORD X_image, DWORD Y_image)
{
	int i;
	buf = jpg_buf0;
	...
	load_JPEG_header(buf, length_of_file, &X_image, &Y_image);

	PScene_LOGI(" X_image = %d\n", X_image);
	PScene_LOGI(" Y_image = %d\n", Y_image);

	PScene_LOGI("Decoding JPEG image...\n");
	// main decoder   
	start_time = clock();
	decode_JPEG_image();
	PScene_LOGI("Decoding finished.\n");

	finish_time = clock();
	duration = (double)(finish_time - start_time) / CLK_TCK / 1000;
	PScene_LOGI("Time elapsed: %2.3f seconds\n", duration);
	
	if (!get_JPEG_buffer(X_image, Y_image, bmp_buf))
	...
	for (i = 0; i <= 3; i++) 
		free(QT[i]);
#if 1
        write_buf_to_BMP(bmp_buf, X_image, Y_image, (char*)"data/vendor/cameraserver/save.bmp");
#endif ///0
